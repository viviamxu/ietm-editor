...
| **`onInsertSymbolClick`** | 接管「插入符号」（工具栏或 warning/caution/note 默认图标）。未传时打开内置弹框；配置了 **`apiBaseUrl`** 时弹框走 ICN 接口。确认后调用 **`instance.insertSymbol(payload, options?)`**；若 `ctx.attentionBlockPos` 有值，传入 `{ attentionBlockPos: ctx.attentionBlockPos }` 写入块级符号。 |
...
| `insertLevelledPara` / `insertSequentialList` / `insertRandomList` / `insertTable` / `insertImage` / `insertFilm` / `insertSymbol` | 各插入类按钮。 |
| `internalRef` | 内部引用。 |
...
### 在光标处插入符号：`insertSymbol`

```ts
// 行内 symbol（para / lead 等）
instance.insertSymbol({
  src: "https://.../icon.svg",
  alt: "符号说明",
  figureId: "ICN-XXX-000001",
});

// warning/caution/note 块级 symbol（默认图标点选后）
instance.insertSymbol(
  { src: "...", figureId: "ICN-XXX-000002" },
  { attentionBlockPos: ctx.attentionBlockPos },
);
```

payload 字段与 **`InsertImagePayload`** 相同（`src` / `alt` / `figureId`）。自动在行内或 attention 块级路径间选择；也可显式传 **`attentionBlockPos`**。与 **`onInsertSymbolClick`** 组合时由宿主选 ICN 后再调用。

### 内置选图弹框与 ICN 接口

未配置 **`onInsertImageClick` / `onInsertFilmClick` / `onInsertSymbolClick`** 时，SDK 打开内置「插入图片 / 多媒体 / 符号」弹框。创建实例时传入 **`apiBaseUrl`**（及可选 **`icnInfoPath`**）后，弹框列表改走 **`/icn/icnInfo`** 分页接口；未配置时仍使用 mock 数据。
...
```ts
{
  ...
  insertSymbol(payload: InsertImagePayload, options?: InsertSymbolOptions): boolean // 行内或 attention 块级 symbol
  getJSON(): JSONContent
  focus(): void

...
instance.insertImages([
  { src: "https://...", alt: "图注", figureId: "ICN-XXX-000001" },
  // unitOfMeasure 可选，与 IETMImage 扩展一致
]);
```

返回 `boolean`：未就绪或插入失败为 `false`。与 `onInsertImageClick` 组合时，由宿主选图后再调用。
...
```ts
{
  ...
  insertImages(images: InsertImagePayload[]): boolean // 光标处插入 S1000D 图片节点；未就绪 false
  getJSON(): JSONContent
  focus(): void

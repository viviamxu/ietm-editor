...
export function insertTableFromSchema(
  ...
  const includeEmptyTitle =
    tableRule === '' || /\btitle/.test(tableRule)
...
export function insertFilmFromSchema(editor: Editor, schema: DescriptionSchema): void {
  void editor
  void schema
  throw new Error('[ietm-editor] insertFilmFromSchema 尚未实现')
}

export function insertImageFromSchema(editor: Editor, schema: DescriptionSchema): void {
  void schema
